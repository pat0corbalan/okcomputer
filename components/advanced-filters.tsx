...
function Chip({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex w-full items-center justify-between rounded-xl px-3 py-2 text-xs transition-all active:scale-[0.99] border",
        // Convertido a un estilo de botón iluminado tipo neón al activarse
        active
          ? "bg-primary/20 border-primary/40 text-white font-semibold shadow-[0_0_15px_rgba(249,115,22,0.1)]"
          : "bg-transparent border-transparent hover:bg-white/[0.03] text-gray-400 hover:text-white"
      )}
    >
      <span className="truncate">{label}</span>
      <span className={cn(
        "text-[10px] font-mono opacity-60 px-1.5 py-0.5 rounded-md bg-white/[0.02]",
        active && "text-neon-orange opacity-100"
      )}>{count}</span>
    </button>
  )
}
...
function Section({ title, open, onToggle, children, isLoading }: SectionProps) {
  return (
    <div className="border-b border-white/5 py-3 relative">
      <button
        onClick={onToggle}
        className="flex w-full items-center justify-between mb-2 text-gray-300 hover:text-white transition-colors"
      >
      ...
      >
        <div className="overflow-hidden space-y-1 relative min-h-[40px]">
          ...
          {/* LOADING BORROSO ESTILO CRISTAL OSCURIZADO */}
          {isLoading && (
            <div className="absolute inset-0 z-20 flex items-center justify-center rounded-xl bg-[#0a0f1d]/70 backdrop-blur-md transition-all animate-in fade-in duration-150">
              <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#0d1527] shadow-xl border border-white/5 animate-in zoom-in-95 duration-150">
...
export function AdvancedFilters({
  ...
  const handleCategorySelect = (category: string) => {
    ...
    if (!isRemoving) {
      setLoadingSection("cat")
      setPendingNavigation("sub")
    } else {
      setLoadingSection("all")
    }
    startTransition(() => {
  ...
  const handleSubcategorySelect = (subcat: string) => {
    ...
    if (!isRemoving) {
      setLoadingSection("sub")
      setPendingNavigation("brand")
    } else {
      setLoadingSection("all")
    }
    startTransition(() => {
      ...
      <div className="flex flex-wrap gap-2 pb-3">
        {items.map((i) => (
          <button
            key={i.label}
            onClick={i.onRemove}
            className="flex items-center gap-2 text-xs px-3 py-1.5 rounded-full bg-neon-cyan/10 border border-neon-cyan/20 text-neon-cyan shadow-sm hover:bg-neon-cyan/20 transition-all duration-200"
          >
            {i.label}
            <X className="h-3 w-3" />
          </button>
  ...
  return (
    <div className="w-full h-full max-h-[80vh] md:max-h-none overflow-y-auto pr-1 space-y-3 scroll-smooth relative">
      ...
      {/* HEADER STICKY INTEGRADO */}
      <div className="flex items-center justify-between border-b border-white/5 pb-3 sticky top-0 bg-background z-10">
        ...
        {hasFilters && (
          <button
            onClick={clearAll}
            className="flex items-center gap-1 text-xs text-gray-500 hover:text-neon-orange transition-colors"
          >
      ...
      <div className="relative space-y-3">
        ...
        <div>
          ...
          >
            <input
              ...
              className="w-full mb-2 px-3 py-2 text-xs rounded-lg bg-[#121b2e]/60 border border-white/5 focus:ring-1 focus:ring-neon-cyan/40 text-white outline-none placeholder:text-gray-500 font-medium transition-all"
            />
        ...
        {/* OVERLAY GLOBAL DE CRISTAL AL LIMPIAR TODO */}
        {isPending && loadingSection === "all" && (
          <div className="absolute inset-0 z-30 flex items-center justify-center bg-[#0a0f1d]/50 backdrop-blur-xs animate-in fade-in duration-150 rounded-xl" />
        )}
      </div>
    </div>
  )
